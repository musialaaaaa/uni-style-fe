...
const ProductList = ({
    ...
    const [mockProducts, setMockProducts] = useState([
        {
            ...
            description: 'Áo thun nam chất liệu cotton 100%, thoáng mát, form regular fit phù hợp mọi độ tuổi.',
            price: 250000,
            quantity: 100,
            ...
            color: 'Trắng, Đen, Xám',
            material: 'Cotton 100%',
            images: ['https://via.placeholder.com/300x300?text=Áo+thun+nam'],
            isDeleted: false,
            createdBy: 'ADMIN',
            createdAt: '2025-07-14T21:29:46.205544Z',
            updatedBy: 'ADMIN',
            updatedAt: '2025-07-14T21:29:46.205544Z'
        },
        {
            id: 2,
            code: 'SP002',
            name: 'Quần jean nữ slim fit xanh đậm',
            description: 'Quần jean nữ form slim fit ôm dáng, chất liệu denim cao cấp co giãn nhẹ.',
            price: 450000,
            quantity: 75,
            ...
            images: ['https://via.placeholder.com/300x300?text=Quần+jean+nữ'],
            isDeleted: false,
            createdBy: 'ADMIN',
            createdAt: '2025-07-14T21:29:46.205544Z',
            updatedBy: 'ADMIN',
    ...
    const loadApiData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/products?${params}`, {
                ...
                headers: {
                    'accept': '*/*',
                    'Authorization': propToken ? `Bearer ${propToken}` : '',
                },
            });
            ...
            // API response structure: { data: [...], metadata: { page, size, total, totalPage } }
    ...
    const loadDropdownOptions = async () => {
        ...
        try {
            const [categoriesRes, brandsRes, materialsRes, colorsRes, sizesRes] = await Promise.all([
                fetch(`${API_BASE_URL}/category?name=&page=0&size=100`, {
                    headers: {
                        'accept': '*/*',
                        'Authorization': propToken ? `Bearer ${propToken}` : ''
                    }
                }),
                fetch(`${API_BASE_URL}/brands?page=0&size=100`, {
                    headers: {
                        'accept': '*/*',
                        'Authorization': propToken ? `Bearer ${propToken}` : ''
                    }
                }),
                fetch(`${API_BASE_URL}/materials?page=0&size=100`, {
                    headers: {
                        'accept': '*/*',
                        'Authorization': propToken ? `Bearer ${propToken}` : ''
                    }
                }),
                fetch(`${API_BASE_URL}/colors?page=0&size=100`, {
                    headers: {
                        'accept': '*/*',
                        'Authorization': propToken ? `Bearer ${propToken}` : ''
                    }
                }),
                fetch(`${API_BASE_URL}/sizes?page=0&size=100`, {
                    headers: {
                        'accept': '*/*',
                        'Authorization': propToken ? `Bearer ${propToken}` : ''
                    }
            ...
            if (categoriesRes.ok) {
                ...
                // Categories have different structure: { data: [{ name, isDeleted }], metadata: {...} }
            ...
            message.error('Lỗi khi tải danh sách lựa chọn');
        } finally {
    ...
    const saveProductDetailApi = async (productDetailData, isEdit = false) => {
        try {
            const url = isEdit
                ? `${API_BASE_URL}/product-details/${productDetailData.id}`
            ...
            const requestBody = {
                ...
                materialId: productDetailData.materialId,
                brandId: productDetailData.brandId,
                colorId: productDetailData.colorId,
                sizeId: productDetailData.sizeId
            };
            ...
            const result = await response.json();
            message.success(isEdit ? 'Cập nhật chi tiết sản phẩm thành công!' : 'Thêm chi tiết sản phẩm thành công!');
            return result;
        } catch (error) {
    ...
    const saveProductApi = async (productData, isEdit = false) => {
        try {
            ...
            // Include code only when editing
            if (isEdit && productData.code) {
                requestBody.code = productData.code;
            }
    ...
    const updateProductStatusApi = async (productId, isDeleted) => {
        try {
            ...
            const statusText = isDeleted ? 'ngưng bán' : 'kích hoạt';
    ...
    const loadMockData = () => {
        ...
        setTimeout(() => {
            const filteredData = mockProducts.filter(product => {
                const matchCode = !filters.code || product.code.toLowerCase().includes(filters.code.toLowerCase());
                const matchName = !filters.name || product.name.toLowerCase().includes(filters.name.toLowerCase());
                const matchDesc = !filters.description || product.description.toLowerCase().includes(filters.description.toLowerCase());
            ...
            setLoading(false);
            message.success(`Tải thành công ${filteredData.length} sản phẩm mẫu`);
        }, 500);
    };
    ...
    const handleViewDetail = async (product) => {
        try {
            if (useMockData) {
                ...
                setLoading(true);
                const productDetail = await loadProductDetailApi(product.id);

                setViewingProduct(productDetail);
                setDetailModalVisible(true);
                setIsEditingInDetail(false);

                // Set form values with product detail data
                detailForm.setFieldsValue({
                    ...
                    categoryId: productDetail.category?.name, // Use name instead of id for category
                    materialId: productDetail.material?.id,
    ...
    const handleSaveDetailEdit = async () => {
        try {
            ...
            console.log('Save detail edit:', values);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            if (editingProduct) {
                ...
                // Browser alert as ultimate fallback
                setTimeout(() => {
                ...
                const { code, ...dataWithoutCode } = values;
                console.log('🔥 Data to send:', dataWithoutCode);

                const result = await saveProductApi(dataWithoutCode, false);
            ...
            // Browser alert as ultimate fallback
            setTimeout(() => {
                alert(`❌ ${editingProduct ? 'Cập nhật' : 'Thêm'} sản phẩm thất bại! ${error.message || ''}`);
            }, 1000);
        } finally {
            setLoading(false);
        }
                ...
                footer={[
                    ...
                    !isEditingInDetail ? (
                        ...
                        <Space key="edit-actions">
                            <Button
                                icon={<CloseOutlined />}
                                onClick={handleCancelEditInDetail}
                            >
                                Hủy
                            </Button>
                            <Button
                        ...
                        <Row gutter={24}>
                            <Col span={8}>
                                <div style={{ textAlign: 'center' }}>
                                    <img
                                        src={viewingProduct.image || 'https://via.placeholder.com/300x300?text=No+Image'}
                            ...
                            <Col span={16}>
                                <Row gutter={[16, 16]}>
                                    <Col span={12}>
                                        <Form.Item
                                            label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Mã sản phẩm</span>}
                                            name="code"
                                        >
                                            <Input
                                                disabled={true}
                                    ...
                                    <Col span={12}>
                                        <Form.Item
                                            label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Tên sản phẩm</span>}
                                            name="name"
                                    ...
                                    {!useMockData && (
                                        <>
                                            <Col span={12}>
                                                <Form.Item
                                                    label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Giá (VNĐ)</span>}
                                                    name="price"
                                            ...
                                            <Col span={12}>
                                                <Form.Item
                                                    label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Số lượng</span>}
                                                    name="quantity"
                                            ...
                                            <Col span={12}>
                                                {isEditingInDetail ? (
                                                    <Form.Item
                                                        label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Danh mục</span>}
                                                        name="categoryId"
                                            ...
                                            <Col span={12}>
                                                {isEditingInDetail ? (
                                                    <Form.Item
                                                        label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Thương hiệu</span>}
                                                        name="brandId"
                                            ...
                                            <Col span={12}>
                                                {isEditingInDetail ? (
                                                    <Form.Item
                                                        label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Chất liệu</span>}
                                                        name="materialId"
                                            ...
                                            <Col span={12}>
                                                {isEditingInDetail ? (
                                                    <Form.Item
                                                        label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Màu sắc</span>}
                                                        name="colorId"
                                            ...
                                            <Col span={12}>
                                                {isEditingInDetail ? (
                                                    <Form.Item
                                                        label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Kích thước</span>}
                                                        name="sizeId"
                                    ...
                                    <Col span={24}>
                                        {isEditingInDetail ? (
                                            <Form.Item
                                                label={<span style={{ color: '#1890ff', fontWeight: 'bold' }}>Trạng thái</span>}
                                                name="isDeleted"
                                            ...
                                            <div style={{ marginBottom: '16px' }}>
                                                ...
                                                <div style={{ marginTop: '8px' }}>
                                                    <Tag color={!viewingProduct.isDeleted ? 'green' : 'red'} style={{ fontSize: '14px' }}>
