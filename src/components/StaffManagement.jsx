...
const StaffManagement = ({ token, currentUser, messageApi }) => {
  ...
  const permissions = ["all", "sales", "customers", "products", "inventory", "reports", "settings"];
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      setStaff(mockStaff);
      messageApi.success("Tải dữ liệu thành công");
    } catch (error) {
  ...
  const handleDelete = async id => {
    if (id === 1) {
      messageApi.error("Không thể xóa tài khoản Admin chính");
      return;
    }
  ...
  const handlePasswordSubmit = async values => {
    ...
    try {
      ...
      messageApi.success(`Đổi mật khẩu cho ${selectedStaff.username} thành công`);
  ...
  const handleStatusChange = async (id, status) => {
    if (id === 1 && status === "inactive") {
      messageApi.error("Không thể vô hiệu hóa tài khoản Admin chính");
      return;
    }
    ...
    try {
      ...
      messageApi.success(
        `${status === "active" ? "Kích hoạt" : "Vô hiệu hóa"} nhân viên thành công`,
      );
    } catch (error) {
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="salary" label="Lương (VNĐ)">
                <Input type="number" placeholder="Nhập mức lương" addonAfter="đ" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="emergency_contact" label="SĐT khẩn cấp">
          ...
          <Form.Item name="address" label="Địa chỉ">
            <TextArea rows={2} placeholder="Nhập địa chỉ..." showCount maxLength={200} />
          </Form.Item>
          ...
          >
            <Select mode="multiple" placeholder="Chọn quyền hạn" allowClear>
        ...
        {selectedStaff && (
          <div>
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col span={6}>
                <Statistic title="Vai trò" value={getRoleInfo(selectedStaff.role).label} />
              </Col>
              <Col span={6}>
                <Statistic title="Phòng ban" value={selectedStaff.department} />
              </Col>
              <Col span={6}>
                <Statistic
                  title="Lương"
