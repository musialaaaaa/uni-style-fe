...
const CustomerManagement = ({ messageApi }) => {
  ...
  const handleSubmit = async values => {
    ...
    try {
      ...
      if (editingCustomer) {
        setCustomers(prev =>
          prev.map(item => (item.id === editingCustomer.id ? { ...item, ...customerData } : item)),
        );
  ...
  const handleStatusChange = async (id, status) => {
    ...
    try {
      ...
      messageApi.success(
        `${status === "active" ? "Kích hoạt" : "Vô hiệu hóa"} khách hàng thành công`,
      );
    } catch (error) {
      ...
      render: (_, record) => (
        <Space>
          ...
          <div>
            ...
            <br />
            <Tag size="small" color={getCustomerLevel(record.total_spent).color}>
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          ...
          <div style={{ marginBottom: 4 }}>
            <Text style={{ color: "#52c41a" }}>{record.total_spent.toLocaleString()}đ</Text>
          </div>
          <div>
          ...
          <div>
            ...
            <Row gutter={16}>
              ...
              <Col span={12}>
                ...
                <div style={{ marginBottom: 12 }}>
                  <Text strong>Hạng thành viên:</Text>
                  <br />
