...
const SalesManagement = ({ currentUser }) => {
    ...
    const applyCoupon = (couponCode) => {
        ...
        if (subtotal < coupon.minOrder) {
            message.error(`Đơn hàng tối thiểu ${coupon.minOrder.toLocaleString()}đ để sử dụng mã này`);
            return;
        }
    ...
    const handlePayment = async (values) => {
        ...
        try {
            ...
            message.success(`Thanh toán thành công! Tiền thừa: ${(values.receivedAmount - total).toLocaleString()}đ`);
        } catch (error) {
        ...
        },
        {
            ...
            render: (_, record) => (
                <div>
                    ...
                    <br />
                    <Text type="secondary" style={{ fontSize: 11 }}>{record.material}</Text>
                </div>
            ),
        },
        {
                                                ...
                                                <Col span={9}>
                                                    <div style={{ fontSize: 12 }}>
                                                        ...
                                                        {item.selectedSize && item.selectedSize !== 'OneSize' && (
                                                            <Tag size="small" color="blue">Size: {item.selectedSize}</Tag>
                                                        )}
                            ...
                            <>
                                <div style={{
                                    ...
                                    <Row justify="space-between" style={{ marginBottom: 4 }}>
                                        <Text style={{ fontSize: 13 }}>Tạm tính ({cart.reduce((sum, item) => sum + item.quantity, 0)} sản phẩm):</Text>
                                        <Text style={{ fontSize: 13 }}>{subtotal.toLocaleString()}đ</Text>
                                    </Row>
                                    {discount > 0 && (
                                        <Row justify="space-between" style={{ marginBottom: 4 }}>
                                            <Text style={{ fontSize: 13 }}>Giảm giá:</Text>
                                            <Text style={{ color: '#52c41a', fontSize: 13 }}>-{discount.toLocaleString()}đ</Text>
                                        </Row>
                                    )}
                        ...
                        renderItem={(item) => (
                            <List.Item>
                                <List.Item.Meta
                                    ...
                                    title={`${item.name} ${item.selectedSize !== 'OneSize' ? `(${item.selectedSize})` : ''}`}
                                    description={`${item.quantity} x ${item.price.toLocaleString()}đ = ${(item.quantity * item.price).toLocaleString()}đ`}
                                />
                            </List.Item>
                        )}
                    />
